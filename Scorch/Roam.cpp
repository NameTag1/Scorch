...
void Roam::update(sf::Time dt, CommandQueue& Commands, SceneNode& target)
{
	...
	if (!originSet) {
		originX = mTarget->getWorldPosition().x;
		originSet = true;
		...
		if (paused) {
			...
			actionInterval = sf::seconds(static_cast<float>(intervalDist(rng)));
			movingRight = (currentTargetX > mTarget->getWorldPosition().x);
		}
	...
	if (playerRange != -1) {
		...
		if (std::abs(playerPos.x - mTarget->getWorldPosition().x) < playerRange) {
			isFinished = true;
		}
	}
	...
	if (paused) {
		...
		double currentX = mTarget->getWorldPosition().x;
